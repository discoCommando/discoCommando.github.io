...
Elm.Cztery = Elm.Cztery || {};
Elm.Cztery.make = function (_elm) {
   "use strict";
   _elm.Cztery = _elm.Cztery || {};
   if (_elm.Cztery.values) return _elm.Cztery.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var setSiteTask = function (s) {
      return $Http.getString(A2($Http.url,
      "http://students.mimuw.edu.pl/~tw336034/ktoPlaci/server.php",
      _U.list([{ctor: "_Tuple2",_0: "mode",_1: "SET"},{ctor: "_Tuple2",_0: "content",_1: s}])));
   };
   var getSiteTask = $Http.getString(A2($Http.url,
   "http://students.mimuw.edu.pl/~tw336034/ktoPlaci/server.php",
   _U.list([{ctor: "_Tuple2",_0: "mode",_1: "GET"}])));
   var ZatwierdźButtonClick = {ctor: "ZatwierdźButtonClick"};
   var CofnijButtonClick = {ctor: "CofnijButtonClick"};
   var ButtonClick = function (a) {    return {ctor: "ButtonClick",_0: a};};
   var GetSite = function (a) {    return {ctor: "GetSite",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var inbox = $Signal.mailbox(NoOp);
   var sendToInbox = function (s) {    var foo = A2($Debug.log,s,s);return A2($Signal.send,inbox.address,GetSite(s));};
   var getAndSendSite = Elm.Native.Task.make(_elm).perform(A2($Task.andThen,getSiteTask,sendToInbox));
   var viewList = F2(function (address,l) {
      var viewSingle = function (wpis) {
         return A2($Html.tr,
         _U.list([]),
         _U.list([A2($Html.th,_U.list([]),_U.list([$Html.text($Basics.toString(wpis.nr))]))
                 ,A2($Html.th,_U.list([]),_U.list([$Html.text(wpis.kto)]))
                 ,A2($Html.th,_U.list([]),_U.list([$Html.text($Basics.toString(wpis.ile))]))
                 ,A2($Html.th,
                 _U.list([]),
                 _U.list([A2($Html.button,
                 _U.list([A2($Html$Events.onClick,address,ButtonClick(wpis.kto)),$Html$Attributes.$class("btn btn-primary btn-sm")]),
                 _U.list([$Html.text("Zapłacił")]))]))]));
      };
      return A2($List.map,viewSingle,l);
   });
   var viewTable = F2(function (address,model) {
      ...
      _U.list([A2($Html.thead,
              ...
              _U.list([A2($Html.th,_U.list([]),_U.list([$Html.text("#")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Kto")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Ile")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Akcja")]))]))]))
              ,A2($Html.tbody,_U.list([]),A2(viewList,address,model.list))]));
   });
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.div,
              ...
              _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("col-md-6 col-md-offset-3")]),_U.list([A2(viewTable,address,model)]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("row")]),
              _U.list([A2($Html.div,
                      _U.list([$Html$Attributes.$class("col-md-3 col-md-offset-3")]),
                      _U.list([A2($Html.a,
                      $List.isEmpty(model.lastLists) ? _U.list([$Html$Attributes.$class("btn btn-lg btn-default disabled")]) : _U.list([A2($Html$Events.onClick,
                                                                                                                                       address,
                                                                                                                                       ZatwierdźButtonClick)
                                                                                                                                       ,$Html$Attributes.$class("btn btn-lg btn-default")]),
                      _U.list([$Html.text("Zatwierdź")]))]))
                      ,A2($Html.div,
                      _U.list([$Html$Attributes.$class("col-md-4 ")]),
                      _U.list([A2($Html.a,
                      $List.isEmpty(model.lastLists) ? _U.list([$Html$Attributes.$class("btn btn-lg btn-danger disabled")]) : _U.list([A2($Html$Events.onClick,
                                                                                                                                      address,
                                                                                                                                      CofnijButtonClick)
                                                                                                                                      ,$Html$Attributes.$class("btn btn-lg btn-danger  ")]),
                      _U.list([$Html.text("Cofnij")]))]))]))]));
   });
   var model = {temp: "NIC",list: _U.list([]),lastLists: _U.list([])};
   var init = {ctor: "_Tuple2",_0: model,_1: $Effects.none};
   var ponumeruj = function (l) {
      return A2($List.map,
      function (wpis) {
         return _U.update(wpis,{nr: wpis.nr + 1});
      },
      A2($List.indexedMap,F2(function ($int,wpis) {    return _U.update(wpis,{nr: $int});}),l));
   };
   var sortFunction = function (a) {
      return ponumeruj(A2($List.sortBy,function (_) {    return _.ile;},A2($List.sortBy,function (_) {    return _.kto;},a)));
   };
   var unconvertLine = function (w) {    return A2($Basics._op["++"],w.kto,A2($Basics._op["++"],",",$Basics.toString(w.ile)));};
   var getRawFromModel = function (l) {    return $String.concat(A2($List.intersperse,"\n",A2($List.map,unconvertLine,l)));};
   var Wpis = F3(function (a,b,c) {    return {nr: a,kto: b,ile: c};});
   var convertLine = function (str) {
      var arr = A2($String.split,",",str);
      var fst = A2($Maybe.withDefault,"Bartek",$List.head(arr));
      var snd$ = $String.toInt(A2($Maybe.withDefault,"0",$List.head(A2($List.drop,1,arr))));
      var snd = function () {    var _p0 = snd$;if (_p0.ctor === "Ok") {    return _p0._0;} else {    return 0;}}();
      return A3(Wpis,0,fst,snd);
   };
   var getModelFromRaw = function (s) {    return sortFunction(A2($List.map,convertLine,$String.lines($String.trim(s))));};
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "NoOp": return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         case "GetSite": var _p2 = _p1._0;
           return {ctor: "_Tuple2",_0: _U.update(model,{temp: _p2,list: getModelFromRaw(_p2)}),_1: $Effects.none};
         case "ButtonClick": return {ctor: "_Tuple2"
                                    ,_0: _U.update(model,
                                    {list: sortFunction(A2($List.map,
                                    function (wpis) {
                                       return _U.eq(wpis.kto,_p1._0) ? _U.update(wpis,{ile: wpis.ile + 1}) : wpis;
                                    },
                                    model.list))
                                    ,lastLists: A2($Basics._op["++"],_U.list([model.list]),model.lastLists)})
                                    ,_1: $Effects.none};
         case "CofnijButtonClick": return {ctor: "_Tuple2"
                                          ,_0: _U.update(model,
                                          {list: A2($Maybe.withDefault,model.list,$List.head(model.lastLists)),lastLists: A2($List.drop,1,model.lastLists)})
                                          ,_1: $Effects.none};
         default: return {ctor: "_Tuple2"
                         ,_0: model
                         ,_1: $Effects.task(A2($Task.andThen,
                         A2($Task.onError,setSiteTask(getRawFromModel(model.list)),function (_p3) {    return $Task.succeed("");}),
                         function (_p4) {
                            return $Task.succeed(NoOp);
                         }))};}
   });
   var app = $StartApp.start({init: init,update: update,view: view,inputs: _U.list([inbox.signal])});
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var main = app.html;
   var Model = F3(function (a,b,c) {    return {temp: a,list: b,lastLists: c};});
   return _elm.Cztery.values = {_op: _op
                               ,Model: Model
                               ,Wpis: Wpis
                               ,convertLine: convertLine
                               ,unconvertLine: unconvertLine
                               ,ponumeruj: ponumeruj
                               ,sortFunction: sortFunction
                               ,getModelFromRaw: getModelFromRaw
                               ,getRawFromModel: getRawFromModel
                               ,model: model
                               ,init: init
                               ,viewList: viewList
                               ,viewTable: viewTable
                               ...
                               ,CofnijButtonClick: CofnijButtonClick
                               ,ZatwierdźButtonClick: ZatwierdźButtonClick
                               ,update: update
                               ,inbox: inbox
                               ,getSiteTask: getSiteTask
                               ,sendToInbox: sendToInbox
                               ,setSiteTask: setSiteTask
                               ,app: app
                               ,main: main};
};
